...
import static PracticeIrinaZ.takeout.utils.PrintUtils.PrintTitles.*;
...
public class Menu {
    ...
    Side beens = new Side("Green Beens Amandine", 5.00);
    ...
    Drink whineRed = new Drink("Chateau Corton Grancey Grand Cru", 40.00);
    ...
    Drink[] drinks = new Drink[]{water, wineWhite, whineRed, juiceApple};
    ...
    private void printMainDishes() {
        printMainDishesTitles();
        for (int i = 0; i < mainDishesNames.length; i++) {
            print(mainDishesNames[i]);
            printSpacesAfterName(spacesAfterName(mainDishesNames,
                    getSidesNames(),
                    getDrinksNames(),
                    getMainDishesNames()[i]));
